...
K = [
    ...
    0xc6e00bf33da88fc2, 0xd5a79147930aa725, # ✅ corrigé
    0x06ca6351e003826f, 0x142929670a0e6e70,
    0x27b70a8546d22ffc, 0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed, 0x53380d139d95b3df,
    0x650a73548baf63de, 0x766a0abb3c77b2a8, # ✅ corrigé
    0x81c2c92e47edaee6, 0x92722c851482353b,
    0xa2bfe8a14cf10364, 0xa81a664bbc423001, 0xc24b8b70d0f89791, 0xc76c51a30654be30,
    0xd192e819d6ef5218, 0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8,
    0x19a4c116b8d2d0c8, 0x1e376c085141ab53, # ✅ corrigé
...
def sha512_process_chunk(chunk, h):
    ...
    return [(h[i]+v)&M for i,v in enumerate([a,b,c,d,e,f,g,hh])]
...
def SHA512(message):
    ...
    while len(msg) % 128 != 112:
        msg += b'\x00'
    msg += struct.pack('>QQ', 0, ml)

    for i in range(0, len(msg), 128):
        h = sha512_process_chunk(bytes(msg[i:i+128]), h)
    ...
    while True:
        ...
        try:
            if choice == '1':
                ...
                ref     = hashlib.sha512(message.encode()).hexdigest()
                print(f"\nSHA-512 (impl) : {result}")
                print(f"SHA-512 (lib)  : {ref}")
                print("✓ Identiques" if result == ref else "✗ Différents")
                ...
                with open(filepath, 'rb') as f:
                    data = f.read()
                print(f"\nFile   : {filepath}")
                print(f"SHA-512: {SHA512(data)}")

            elif choice == '3':
                message  = input("Enter message: ")
                computed = SHA512(message)
                expected = input("Enter expected hash: ").lower()
                print(f"\nComputed: {computed}")
                print(f"Expected: {expected}")
                print("✓ MATCH" if computed == expected else "✗ NO MATCH")
